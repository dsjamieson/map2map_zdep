...
class ConvStyled3d(nn.Module):
    ...
    def forward(self, x, s, eps=1e-8):
        #print("    CS forward: %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        N, Cin, *DHWin = x.shape
        #print("    CS shapes : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        C0, C1, *K3 = self.weight.shape
        #print("    CS weights: %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        if self.resample == 'U':
            Cin, Cout = C0, C1
        else:
            Cout, Cin = C0, C1
        ...
        #print("    CS style  : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        ...
        #print("    CS modulat: %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))

        w = self.weight * s
        #print("    CS weight : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        ...
        #print("    CS demod : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        ...
        #print("    CS out   : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024), x.shape)
