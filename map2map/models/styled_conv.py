...
class ConvStyledBlock(nn.Module):
    ...
    def forward(self, x, s):
        #print("    CN forward: %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
        for count, l in enumerate(self.convs) :
            x = l(x, s)
            #print("    CN conv %d : %.2e GB" % (count, torch.cuda.memory_allocated(0)/1024/1024/1024))
        return x
...
class ResStyledBlock(ConvStyledBlock):
    ...
    def forward(self, x, s):
        ...
        #print("  RN forward: %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))

        y = x

        #print("  RN copy   : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))

        if self.skip is not None:
            y = self.skip(y, s)
            #print("  RN skip   : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024), x.shape, y.shape)

        for count, l in enumerate(self.convs) :
            x = l(x, s)
            #print("  RN layer %d: %.2e GB" % (count, torch.cuda.memory_allocated(0)/1024/1024/1024))

        y = narrow_like(y, x)
        x += y
        #print("  RN narrow : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))

        if self.act is not None:
            x = self.act(x, s)

        #print("  RN act    : %.2e GB" % (torch.cuda.memory_allocated(0)/1024/1024/1024))
